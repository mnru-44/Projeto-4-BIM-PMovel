...
import { pegarHidratantes } from '../../servicos/firestore'
import { Capitalize } from '../../servicos/formatarString';
import { useNavigation } from '@react-navigation/native';

export function ScrollViewHidratantes() {
	...
	const [Hidratantes, setHidratantes] = useState([]);

	useEffect(() => {
		pegarHidratantes(setHidratantes)
	}, [])

	return (
		<ScrollView showsHorizontalScrollIndicator={false} horizontal={true}>
			<View style={{ flexDirection: 'row', left: 20 }}>
					{Hidratantes.map((item, index) => (
